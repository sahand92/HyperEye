...
elseif nargout<3 % faster:
    [~,ndx] = utils.natsort(ext,varargin{:});
    [~,ids] = utils.natsort(fnm(ndx),varargin{:});
else % for debugging:
    [~,ndx,dbg{num+2}] = utils.natsort(ext,varargin{:});
    [~,ids,tmp] = utils.natsort(fnm(ndx),varargin{:});
    [~,idd] = sort(ndx);
    dbg{num+1} = tmp(idd,:);
end
ndx = ndx(ids);
...
for k = num:-1:1
    ...
    if nargout<3 % faster:
        [~,ids] = utils.natsort(vec(ndx),varargin{:});
    else % for debugging:
        [~,ids,tmp] = utils.natsort(vec(ndx),varargin{:});
        [~,idd] = sort(ndx);
        dbg{k} = tmp(idd,:);
    end
    ndx = ndx(ids);
